...
// Sync server VPS IP — used to identify infrastructure connections
export const SYNC_SERVER_HOST = '206.245.132.26'
...
export class SyncClient {
  ...
  async _doInit() {
    ...
    if (this.feed.markInfrastructureHost) {
      this.feed.markInfrastructureHost(SYNC_SERVER_HOST)
    }
  ...
  _isServerConnection(conn, info) {
    ...
    if (host && host === SYNC_SERVER_HOST) return true
    return false
  }
