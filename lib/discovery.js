...
export class Discovery {
  ...
  _handleMessage(msg, conn) {
    if (msg.type === 'discovery-hello' && msg.swarmId) {
      ...
      if (msg.swarmId === this.mySwarmId) {
        if (msg.pubkey === this.identity.pubkeyHex &&
            typeof msg.instanceId === 'string' &&
            msg.instanceId.length <= 64 &&
            msg.instanceId !== this.instanceId &&
            !this._duplicateInstanceSeen.has(msg.instanceId)) {
          this._duplicateInstanceSeen.add(msg.instanceId)
          try { this.onDuplicateInstance?.({ instanceId: msg.instanceId }) } catch (_) {}
        }
        return
      }
