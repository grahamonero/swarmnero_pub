...
export class Discovery {
  constructor(swarm, identity) {
    ...
    // Fires when another live instance of THIS account is detected on the
    // network (same pubkey, different instanceId). Hypercore is single-
    // writer, so running two instances simultaneously causes post failures.
    this.onDuplicateInstance = null

    // Random per-run identifier so we can tell our own echoes vs. a genuine
    // second device signed into the same account. Not signed — a bad actor
    // would need our secret key to forge a hello with our pubkey anyway.
    this.instanceId = _randomNonceHex().slice(0, 16)
    this._duplicateInstanceSeen = new Set()
  ...
  /**
   * setProfile accepts the user's profile and their Hypercore swarmId.
   * If we already have open Discovery channels that failed to send their
   * initial hello (because profile was not yet set), re-send now so we
   * don't wait 10-15s for the retry interval.
   */
  setProfile(profile, swarmId) {
    const prevProfile = this.myProfile
    const prevSwarmId = this.mySwarmId
    this.myProfile = profile
    this.mySwarmId = swarmId

    const becameReady = (!prevProfile || !prevSwarmId) && profile && swarmId
    console.log('[Discovery] setProfile called', {
      hasProfile: !!profile,
      swarmId: swarmId?.slice(0, 12),
      becameReady,
    ...
    // Only swarmId is required; profile can still be null (empty profile is
    // broadcast and duplicate-instance detection works).
    if (swarmId && this.enabled && this.swarm?.connections) {
  ...
  _sendHello(conn) {
    ...
    // Require swarmId + identity (not profile). Duplicate-instance detection
    // and basic presence still work without a saved profile; peers just see
    // our pubkey and swarmId until our profile event replicates.
    if (!this.mySwarmId || !this.identity?.pubkeyHex) {
      console.log('[Discovery] Cannot send hello: swarmId/identity missing')
      return
    }

    const bloomFilter = this.getBloomFilter()
    const stats = this._myStats || {}
    const profileSrc = this.myProfile || {}
    const payload = {
      type: 'discovery-hello',
      swarmId: this.mySwarmId,
      pubkey: this.identity.pubkeyHex,
      profile: {
        name: _cap(profileSrc.name, MAX_NAME),
        bio: _cap(profileSrc.bio, MAX_BIO),
        avatar: _cap(profileSrc.avatar, MAX_AVATAR),
        website: _cap(profileSrc.website, MAX_WEBSITE)
      },
    ...
    console.log('[Discovery] hello sent', {
      mySwarmId: this.mySwarmId?.slice(0, 12),
  ...
  _handleMessage(msg, conn) {
    if (msg.type === 'discovery-hello' && msg.swarmId) {
      const sameSwarm = msg.swarmId === this.mySwarmId
      console.log('[Discovery] hello received', {
        fromSwarmId: msg.swarmId?.slice(0, 12),
        mySwarmId: this.mySwarmId?.slice(0, 12),
        sameSwarm,
        remoteInstanceId: msg.instanceId,
        myInstanceId: this.instanceId
      })
      // Our own swarmId — either an echo or a second live instance signed
      // into this account. Hypercore is single-writer; warn the user.
      if (sameSwarm) {
        const pubkeyMatches = msg.pubkey === this.identity.pubkeyHex
        const instanceIdOk = typeof msg.instanceId === 'string' && msg.instanceId.length <= 64
        const differentInstance = msg.instanceId !== this.instanceId
        const alreadySeen = this._duplicateInstanceSeen.has(msg.instanceId)
        console.log('[Discovery] same-swarm hello details', {
          pubkeyMatches,
          instanceIdOk,
          differentInstance,
          alreadySeen,
          hasCallback: !!this.onDuplicateInstance
        })
        if (pubkeyMatches && instanceIdOk && differentInstance && !alreadySeen) {
          this._duplicateInstanceSeen.add(msg.instanceId)
          console.log('[Discovery] DUPLICATE INSTANCE detected — firing callback')
          try { this.onDuplicateInstance?.({ instanceId: msg.instanceId }) } catch (err) {
            console.error('[Discovery] onDuplicateInstance threw:', err)
          }
        }
        return
      }
      ...
      // Signature must be over the EXACT payload shape the sender signed —
      // same capping (_cap), same slicing. Any divergence in JSON output
      // breaks the Ed25519 check.
      try {
        const signedFields = {
          swarmId: msg.swarmId,
          pubkey: msg.pubkey,
          ...
          postCount,
          following: (Array.isArray(msg.following) ? msg.following : []).slice(0, MAX_FOLLOW_LIST),
          followers: (Array.isArray(msg.followers) ? msg.followers : []).slice(0, MAX_FOLLOW_LIST),
  ...
  _retryAllConnections() {
    ...
    for (const conn of this.swarm.connections) {
      ...
      } else if (!conn._discoveryPeerSwarmId && this.mySwarmId) {
        // Channel is open but no hello exchanged yet — retry. Only swarmId is
        // required (profile can be null for fresh accounts).
        this._sendHello(conn)
      }
    }
  }
