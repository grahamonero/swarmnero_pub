...
export class FoFCache {
  ...
  /**
   * Remove all cached posts authored by a given pubkey.
   * Used when a direct follow's feed is pruned so FoF search doesn't
   * resolve to posts we no longer have.
  ...
  pruneByPubkey(pubkey) {
    ...
    this.posts = this.posts.filter(p => p.pubkey !== pubkey)
    const removed = before - this.posts.length

    if (removed > 0) {
      this._rebuildIndexes()
      this._save()
      console.log(`[FoFCache] Pruned ${removed} posts for pubkey ${pubkey.slice(0, 8)}...`)
    }
