...
/**
 * Validate one media descriptor. Rejects missing/oversized/path-mangled items
 * so a malformed peer event cannot cause renderer / Hyperdrive fetch surprises.
 */
function isValidMediaItem(m) {
  ...
  if (m.thumb !== undefined && m.thumb !== null) {
    // thumb in media descriptor is either an inline data URL (image) or omitted.
...
/**
 * Return the media array filtered to valid entries and truncated to the cap.
 * Use at ingest so a malicious peer can't balloon post size with junk media
 * descriptors or escape Hyperdrive path validation.
 */
