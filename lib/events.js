...
export const EventType = {
  ...
  POLL: 'poll',
  POLL_VOTE: 'poll_vote',
  // Article event type (Phase 2A — long-form posts)
  ARTICLE: 'article',
...
// ---------------------------------------------------------------------------
// Article event (Phase 2A long-form posts) — bounds enforced at compose AND
// ingest. Drops oversized / future-dated / shape-malformed articles silently.
// ---------------------------------------------------------------------------
export const ARTICLE_MAX_TITLE = 200
export const ARTICLE_MAX_SUMMARY = 500
export const ARTICLE_MAX_BODY = 50000
export const ARTICLE_MAX_CW = 200
export const ARTICLE_MAX_TAGS = 20
export const ARTICLE_MAX_TAG_LEN = 64
export const ARTICLE_MAX_PAYWALL_ENC = 80000   // ~50000 chars * ~1.6 base64 expansion
export const ARTICLE_MAX_TOTAL_BYTES = 131072  // 128 KB on the signed-JSON wire
export const ARTICLE_FUTURE_SKEW_MS = 5 * 60 * 1000
...
/**
 * Create a long-form article event (Phase 2A).
 *
 * `body` is markdown source — sanitized at render time by
 * ui/utils/markdown.js. Bounds are enforced here AND on ingest.
 *
 * If `paywall` is provided, the body is replaced by `paywall.encrypted`
 * (ciphertext from lib/paywall.js createPaywalledPost) and the event carries
 * the same paywall_* fields as paywalled posts.
 *
 * Throws on cap violations / malformed input.
 *
 * @param {Object} args
 * @param {string} args.title
 * @param {string} [args.summary]
 * @param {{driveKey:string,path:string}} [args.cover]
 * @param {string} args.body
 * @param {string[]} [args.tags]
 * @param {string} [args.cw]
 * @param {Object} [args.paywall] - { price, preview, encrypted, subaddress, subaddressIndex }
 */
export function createArticleEvent({ title, summary, cover, body, tags, cw, paywall }) {
  if (typeof title !== 'string') throw new Error('Article title is required')
  const trimmedTitle = title.trim()
  if (trimmedTitle.length === 0) throw new Error('Article title cannot be empty')
  if (trimmedTitle.length > ARTICLE_MAX_TITLE) {
    throw new Error(`Article title exceeds ${ARTICLE_MAX_TITLE} chars`)
  }

  let trimmedSummary = ''
  if (summary != null) {
    if (typeof summary !== 'string') throw new Error('Article summary must be a string')
    trimmedSummary = summary.trim()
    if (trimmedSummary.length > ARTICLE_MAX_SUMMARY) {
      throw new Error(`Article summary exceeds ${ARTICLE_MAX_SUMMARY} chars`)
    }
  }

  let normalizedCover = null
  if (cover) {
    if (!isValidMediaItem(cover)) {
      throw new Error('Article cover is malformed (must be {driveKey, path})')
    }
    if (!cover.path.startsWith('/images/')) {
      throw new Error('Article cover must be an /images/ path')
    }
    normalizedCover = {
      driveKey: cover.driveKey,
      path: cover.path
    }
    if (cover.thumb) normalizedCover.thumb = cover.thumb
  }

  let bodyOrEncrypted = ''
  if (paywall && paywall.encrypted) {
    // Paywalled article: body field carries no plaintext; ciphertext lives in
    // paywall_encrypted. We still set a body field (empty) for shape-uniformity.
    bodyOrEncrypted = ''
  } else {
    if (typeof body !== 'string') throw new Error('Article body is required')
    if (body.length > ARTICLE_MAX_BODY) {
      throw new Error(`Article body exceeds ${ARTICLE_MAX_BODY} chars`)
    }
    bodyOrEncrypted = body
  }
  ...
  if (Array.isArray(tags)) {
    for (const t of tags) {
      ...
      if (tag.length === 0 || tag.length > ARTICLE_MAX_TAG_LEN) continue
      if (cleanTags.length >= ARTICLE_MAX_TAGS) break
      cleanTags.push(tag)
    }
  }
  ...
  const event = {
    ...
    body: bodyOrEncrypted,
    tags: cleanTags
  }

  if (typeof cw === 'string') {
    const trimmedCw = cw.trim()
    if (trimmedCw.length > 0 && trimmedCw.length <= ARTICLE_MAX_CW) {
      event.cw = trimmedCw
    }
  }

  if (paywall && paywall.encrypted) {
    if (!paywall.price) throw new Error('Paywall price required')
    event.paywall_price = String(paywall.price)
    event.paywall_preview = paywall.preview || ''
    event.paywall_encrypted = paywall.encrypted
    event.paywall_subaddress = paywall.subaddress || null
    event.paywall_subaddress_index = paywall.subaddressIndex ?? null
  }
...
/**
 * Validate an article event on ingest. Returns true iff structurally sound,
 * within bounds, and not absurdly future-dated.
 *
 * Caller is responsible for calling verifyEventSignature separately
 * (mirrors the poll / paywall pattern). Does NOT verify signature here.
 */
export function validateArticleEvent(event, nowMs = Date.now()) {
  if (!event || event.type !== EventType.ARTICLE) return false
  if (typeof event.pubkey !== 'string' || !/^[a-f0-9]{64}$/i.test(event.pubkey)) return false
  if (!Number.isFinite(Number(event.timestamp))) return false
  if (Number(event.timestamp) > nowMs + ARTICLE_FUTURE_SKEW_MS) return false

  // Title required, bounded
  if (typeof event.title !== 'string' || event.title.length === 0) return false
  if (event.title.length > ARTICLE_MAX_TITLE) return false

  // Summary optional, bounded
  if (event.summary != null) {
    if (typeof event.summary !== 'string') return false
    if (event.summary.length > ARTICLE_MAX_SUMMARY) return false
  }

  // Body bounded (may be empty for paywalled)
  if (typeof event.body !== 'string') return false
  if (event.body.length > ARTICLE_MAX_BODY) return false

  // Cover optional, must validate as a media item with /images/ prefix
  if (event.cover != null) {
    if (!isValidMediaItem(event.cover)) return false
    if (!event.cover.path.startsWith('/images/')) return false
  }

  // Tags array bounded
  if (event.tags != null) {
    if (!Array.isArray(event.tags)) return false
    if (event.tags.length > ARTICLE_MAX_TAGS) return false
    for (const t of event.tags) {
      if (typeof t !== 'string') return false
      if (t.length === 0 || t.length > ARTICLE_MAX_TAG_LEN) return false
    }
  }
  ...
  if (event.cw != null) {
    if (typeof event.cw !== 'string') return false
    if (event.cw.length > ARTICLE_MAX_CW) return false
  }

  // Paywall ciphertext bounded; mutually exclusive with a non-empty plaintext
  // body (otherwise an attacker can pad an extra ARTICLE_MAX_BODY of
  // attacker-controlled text past the paywall preview without functional use).
  if (event.paywall_encrypted != null) {
    const encStr = typeof event.paywall_encrypted === 'string'
      ? event.paywall_encrypted
      : JSON.stringify(event.paywall_encrypted)
    if (encStr.length > ARTICLE_MAX_PAYWALL_ENC) return false
    if (typeof event.body === 'string' && event.body.length > 0) return false
  }

  // Total wire size cap.
  try {
    const wireSize = Buffer.byteLength(JSON.stringify(event), 'utf8')
    if (wireSize > ARTICLE_MAX_TOTAL_BYTES) return false
  } catch {
    return false
  }
...
export function buildThread(events, rootPubkey, rootTimestamp) {
  ...
  // Find the root: post, reply, or article. Articles can be reply roots in
  // panel.js's showThread, so they need to resolve correctly here even though
  // the dedicated article-view.js renderer is preferred for the center column.
  const root = events.find(e =>
    (e.type === EventType.POST || e.type === EventType.REPLY || e.type === EventType.ARTICLE) &&
    e.pubkey === rootPubkey &&
