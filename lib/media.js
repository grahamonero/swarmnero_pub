...
export class Media {
  ...
  async storeImage(file, filename) {
    console.log('[storeImage] begin', { name: filename, type: file.type, size: file.size })
    const timestamp = Date.now()

    const stripped = await stripImageExif(file).catch((err) => {
      console.warn('[storeImage] stripImageExif threw:', err)
      return null
    })
    console.log('[storeImage] stripped:', stripped ? { mimeType: stripped.mimeType, ext: stripped.ext, bytes: stripped.buffer?.byteLength } : null)
    ...
    console.log('[storeImage] putting to drive', { path, bytes: buffer.byteLength })
    await this.drive.put(path, buffer)

    // Verify the put actually landed.
    try {
      const back = await this.drive.get(path)
      console.log('[storeImage] drive.get verify:', back ? `${back.length} bytes` : 'NULL — put did not persist')
    } catch (err) {
      console.warn('[storeImage] verify get failed:', err)
    }
    ...
    console.log('[storeImage] result:', result)
    return result
  }
