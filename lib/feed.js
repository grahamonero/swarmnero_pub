...
import { sanitizeMediaArray } from './events.js'

// Strip any media descriptors that fail the ingest validation (size, path
// safety, thumb shape). Truncates oversize arrays. Never throws — a malformed
// `media` simply becomes an empty one so the rest of the event (content,
// signature) still renders.
...
export class Feed {
  ...
  async read(start = 0, end = this.core.length) {
    ...
    for (let i = start; i < end; i++) {
      ...
      if (data) {
        events.push(sanitizeIngestedEvent(JSON.parse(data.toString())))
      }
    }
    return events
  }
