...
import { sanitizeMediaArray, isValidPostCw, isValidReactionEmoji, validateArticleEvent } from './events.js'

function sanitizeIngestedEvent(event) {
  if (event && (event.type === 'post' || event.type === 'reply') && 'media' in event) {
    event.media = sanitizeMediaArray(event.media)
  }
  return event
}

function passesIngestShapeCheck(event) {
  if (!event || typeof event !== 'object') return false
  if (event.type === 'reaction') {
    return isValidReactionEmoji(event.emoji)
  }
  if (event.type === 'article') {
    return validateArticleEvent(event)
  }
  return true
}
...
export function verifyEventSignature(event) {
  ...
  try {
    // Reconstruct the signed message (event without signature). `_seq` is a
    // local read-time annotation for the Hypercore sequence number and is
    // never part of the signed payload — strip it alongside `signature`.
    const { signature, _seq, ...eventWithoutSig } = event
...
export class Feed {
  constructor(dataDir, identity) {
    ...
    this.onHypercoreKeyPairExtracted = null // async cb fired once after first-launch extraction so the caller can persist
  ...
  async init() {
    ...
    // Prefer the persisted Hypercore keypair so the feed key (swarmId) is
    // portable across devices. If the identity has no keypair saved yet
    // (legacy account pre-v0.9), fall back to the old {name} lookup,
    // extract the keypair from the resulting core, and persist it. After
    // the first launch, all future launches read the keypair from the
    // identity file and skip this branch entirely.
    if (this.identity.hypercoreKeyPair) {
      this.core = this.store.get({ keyPair: this.identity.hypercoreKeyPair })
      await this.core.ready()
    } else {
      this.core = this.store.get({ name: `feed-${this.identity.pubkeyHex}` })
      await this.core.ready()
      try {
        this.identity.setHypercoreKeyPair({
          publicKey: this.core.keyPair.publicKey,
          secretKey: this.core.keyPair.secretKey
        })
        if (this.onHypercoreKeyPairExtracted) {
          await this.onHypercoreKeyPairExtracted(this.identity.hypercoreKeyPair)
        }
      } catch (err) {
        // Extraction is best-effort — if it fails, the user just isn't
        // portable yet. Next launch will retry.
        console.warn('[Feed] Could not extract hypercoreKeyPair:', err.message)
      }
    }
   ...
   *
   * Each returned event is annotated with a non-enumerable-on-wire `_seq`
   * equal to its Hypercore block index. Aggregators (e.g. poll tally) use
   * `_seq` as the tiebreak when two signed events share a wall-clock
   * timestamp. `_seq` is attached AFTER JSON.parse so it is never part of the
   * signed payload and does not affect signature verification.
   */
  async read(start = 0, end = this.core.length) {
    ...
    for (let i = start; i < end; i++) {
      ...
      if (data) {
        ...
        if (!isValidPostCw(event)) continue
        if (!passesIngestShapeCheck(event)) continue
        event._seq = i
        events.push(sanitizeIngestedEvent(event))
      }
    }
    return events
  }
  ...
  /**
   * Purge downloaded block payloads for a peer's feed without unfollowing.
   * Keeps the core + signed tree so blocks can be re-fetched on demand
   * from any peer that still has them.
   *
   * @param {string} swarmIdHex - Follow's swarm ID (hex)
   * @param {number} [start=0]  - Start block (inclusive)
   * @param {number} [end]      - End block (exclusive). Defaults to core.length.
   * @returns {Promise<{cleared: number, bytesEstimated: number}>}
   */
  async clearPeerBlocks(swarmIdHex, start = 0, end) {
    if (typeof swarmIdHex !== 'string' || !/^[a-f0-9]{64}$/i.test(swarmIdHex)) {
      throw new Error('invalid swarmId')
    }
    swarmIdHex = swarmIdHex.toLowerCase()
    const peerCore = this.peers.get(swarmIdHex)
    if (!peerCore) throw new Error('Not following this peer')

    const to = (end === undefined || end === null) ? peerCore.length : end
    const from = Math.max(0, start)
    if (to <= from) return { cleared: 0, bytesEstimated: 0 }

    const bytesBefore = peerCore.byteLength || 0
    await peerCore.clear(from, to)
    const bytesAfter = peerCore.byteLength || 0
    return {
      cleared: to - from,
      bytesEstimated: Math.max(0, bytesBefore - bytesAfter)
    }
  }
    ...
    for (let i = start; i < length; i++) {
      ...
      if (data) {
        ...
        if (event.type === 'poll_vote' &&
            Number(event.timestamp) > Date.now() + 5 * 60 * 1000) {
          continue
        }
        event._seq = i
        events.push(sanitizeIngestedEvent(event))
