...
export class Feed {
  ...
  async readPeer(swarmIdHex, start = 0, end) {
    ...
    for (let i = start; i < length; i++) {
      ...
      if (data) {
        const event = JSON.parse(data.toString())
        // Verify signature before including event. Attach the Hypercore
        // sequence number AFTER verification so the annotation is not part
        // of the signed payload (see read() for rationale).
        if (verifyEventSignature(event)) {
          event._seq = i
          // Drop poll_vote events that are too far in the future at ingest
          // (clock skew tolerance). Keeps malformed events from polluting
          // local tallies. Actual cap vs. poll.expires_at is enforced in
          // tallyPollVotes.
