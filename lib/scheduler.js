...
import { createPostEvent, createPollEvent, POLL_MAX_OPTIONS, POLL_MAX_OPTION_LEN } from './events.js'
...
export class Scheduler {
  ...
  schedule({ payload, sendAt, paywall }) {
    ...
    const kind = payload.kind === 'poll' ? 'poll' : 'post'
    let pollMeta = null
    if (kind === 'poll') {
      const question = typeof payload.poll?.question === 'string' ? payload.poll.question.trim() : ''
      const options = Array.isArray(payload.poll?.options)
        ? payload.poll.options.map(o => String(o).trim()).filter(Boolean).slice(0, POLL_MAX_OPTIONS)
        : []
      const durationMs = Number.isFinite(payload.poll?.durationMs) ? payload.poll.durationMs : 0
      ...
      if (!Number.isFinite(durationMs) || durationMs < 60 * 1000) {
        throw new Error('Invalid poll duration')
      }
      pollMeta = { question, options, durationMs }
    }
    const cw = typeof payload.cw === 'string' && payload.cw.trim() ? payload.cw.trim() : null
    const entry = {
      id: newEntryId(),
      accountId: this.accountId,
      createdAt: now,
      ...
      paywall: kind === 'post' && paywall && typeof paywall === 'object' && paywall.price ? {
  ...
  async _tryFire(entry) {
    ...
    try {
      let appended
      if (entry.kind === 'poll' && entry.payload.poll) {
        const expiresAt = Date.now() + entry.payload.poll.durationMs
        appended = await this.feed.append(createPollEvent({
          question: entry.payload.poll.question,
          options: entry.payload.poll.options,
          expiresAt
        }))
      } else if (entry.paywall) {
