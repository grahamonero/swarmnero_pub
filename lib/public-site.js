...
export async function rebuildPublicSite({ feed, media, identity, dataDir }) {
  ...
  // CSS is now inlined into every page via <style> tags (see pageShell).
  // External style.css files are no longer needed; skip them.

  // Clean up older layouts — external stylesheets are no longer needed.
  try { await media.drive.del(SITE_PATHS.rootStyle) } catch {}
  try { await media.drive.del(SITE_PATHS.style) } catch {}
  ...
  await media.drive.put(SITE_PATHS.root, Buffer.from(profileHtml))
