...
export async function rebuildPublicSite({ feed, media, identity, dataDir }) {
  ...
  // CSS — shared by every page, written once
  await media.drive.put(SITE_PATHS.style, Buffer.from(generateCSS()))
  ...
  await media.drive.put(
    SITE_PATHS.profile,
    Buffer.from(generateProfileHtml(profile, posts, feed.swarmId, events))
  )
