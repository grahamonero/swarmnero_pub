...
const PEAR_LINK = 'pear://y3m1mm1xxrqj9jz5jpicc57rt4dem5t5g9s78j9riqft94u4j6jy'
...
// Wrap rendered body in a <details> CW shell when the post carries a valid cw
// field. Uses native <details>/<summary> so there's no JS dependency on the
// static site. Label is escaped; any cw over 200 chars or non-string is
// treated as absent.
function wrapWithCw(post, innerBodyHtml) {
  const rawCw = (typeof post.cw === 'string') ? post.cw.trim() : ''
  if (!rawCw || rawCw.length > 200) return innerBodyHtml
  return `
    <details class="cw-wrapper">
      <summary class="cw-placeholder">
        <span class="cw-icon">⚠</span>
        <span class="cw-label">${esc(rawCw)}</span>
        <span class="cw-reveal-btn">Show content</span>
      </summary>
      <div class="cw-content">${innerBodyHtml}</div>
    </details>
  `
}
...
.cw-wrapper { margin: 0; }
.cw-wrapper > summary.cw-placeholder { list-style: none; cursor: pointer; display: flex; align-items: center; gap: 10px; padding: 14px; background: rgba(255, 153, 51, 0.08); border: 1px solid rgba(255, 153, 51, 0.3); border-radius: 6px; }
.cw-wrapper > summary.cw-placeholder::-webkit-details-marker { display: none; }
.cw-icon { color: #ff9933; }
.cw-label { flex: 1; color: var(--text); word-break: break-word; }
.cw-reveal-btn { color: #ff9933; border: 1px solid #ff9933; border-radius: 6px; padding: 4px 10px; font-size: 13px; }
.cw-wrapper[open] > summary.cw-placeholder { display: none; }
.cw-content { margin-top: 0; }
