...
export class TagIndex {
  ...
  /**
   * Set the data directory and the owning account's pubkey.
   * Tag-index state is stored per-account so a freshly created account doesn't
   * inherit cached tags from other accounts on the same device.
   * @param {string} dataDir - Data directory path
   * @param {string} [pubkeyHex] - Hex pubkey of the active account. When omitted,
   *                               falls back to the legacy shared filenames so
   *                               early-boot callers don't break.
   */
  setDataDir(dataDir, pubkeyHex = null) {
    this.dataDir = dataDir
    this.pubkeyHex = pubkeyHex || null
  }

  /**
   * Build an account-scoped file path. Falls back to the legacy shared name
   * when no pubkey has been provided so reads from pre-scoped installs still
   * complete (the shared file is subsequently orphaned).
   */
  _scopedPath(stem, ext) {
    return path.join(this.dataDir, this.pubkeyHex ? `${stem}-${this.pubkeyHex}.${ext}` : `${stem}.${ext}`)
  }
  ...
  _getIndexPath() {
    ...
    return this._scopedPath('tag-index', 'json')
  }
  ...
  _getContentPath() {
    ...
    return this._scopedPath('tag-content', 'json')
  }
   ...
   * @param {Set<string>} [deletedKeys] - Optional set of "pubkey:timestamp" keys of deleted posts to exclude
   * @returns {Array} Array of { pubkey, timestamp, source, content } sorted by timestamp (newest first)
   */
  search(tag, deletedKeys = null) {
    ...
    let posts = this.index.get(normalizedTag) || []

    // Filter out deleted posts if a deletedKeys set was provided
    if (deletedKeys && deletedKeys.size > 0) {
      posts = posts.filter(p => !deletedKeys.has(`${p.pubkey}:${p.timestamp}`))
    }
   ...
   */
  getTrending(limit = 10, deletedKeys = null) {
    const tagCounts = []

    for (const [tag, posts] of this.index.entries()) {
      const count = deletedKeys && deletedKeys.size > 0
        ? posts.filter(p => !deletedKeys.has(`${p.pubkey}:${p.timestamp}`)).length
        : posts.length
  ...
  /**
   * Remove all tag entries and cached content for a given author.
   * Used when a follow's feed is pruned so search doesn't resolve to posts we no longer have.
   * @param {string} pubkey - Author pubkey (hex)
   * @returns {number} Number of tag entries removed
   */
  pruneByPubkey(pubkey) {
    if (typeof pubkey !== 'string' || !pubkey) return 0

    let removed = 0
    for (const [tag, posts] of this.index) {
      const filtered = posts.filter(p => p.pubkey !== pubkey)
      const delta = posts.length - filtered.length
      if (delta > 0) {
        removed += delta
        if (filtered.length === 0) {
          this.index.delete(tag)
        } else {
          this.index.set(tag, filtered)
        }
      }
    }

    const prefix = `${pubkey}:`
    for (const key of this.contentStore.keys()) {
