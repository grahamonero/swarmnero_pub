...
export class Identity {
  constructor(dataDir) {
    ...
    this.publicKey = null
    this.secretKey = null
    // Keypair for the account's Hypercore (feed). Populated on first launch
    // from the legacy `{name: ...}` core via extraction. Once persisted, the
    // swarmId is portable across devices — same keypair, same core.key.
    this.hypercoreKeyPair = null
  }
  ...
  setHypercoreKeyPair(kp) {
    if (!kp || !kp.publicKey || !kp.secretKey) {
      throw new Error('hypercoreKeyPair must have publicKey and secretKey')
    }
    this.hypercoreKeyPair = {
      publicKey: b4a.from(kp.publicKey),
      secretKey: b4a.from(kp.secretKey)
    }
  }
