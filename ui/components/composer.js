...
import { createPostEvent, createPollEvent, POLL_MAX_OPTIONS, POLL_MAX_OPTION_LEN } from '../../lib/events.js'
...
export function initComposer(refreshUI) {
  ...
  // Poll toggle - show/hide poll fields and initialise two empty option rows.
  // Mutually exclusive with paywall mode (a poll is its own top-level event).
  const pollToggle = document.getElementById('expPollToggle')
  const pollFields = document.getElementById('expPollFields')
